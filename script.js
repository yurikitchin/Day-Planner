...
function textAreaColour(){
    ...
    if (dateTimeDisplay.format("h a") === "6 am"){
...
var dayTasks = {
    ...
    document.querySelectorAll('.areaPopulate').forEach(item => {
...
function clearFunction () {
    var existingStorage = JSON.parse(localStorage.getItem('user'))
    existingStorage = dayTasks
    localStorage.setItem('user', JSON.stringify(existingStorage))
    location.reload()
}
